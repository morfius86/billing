<?php
/* osmp to simple-billing
(с)Bobrov Nikita
nick.bobrow@gmail.com */

#OPTIONS
define("SENDER", "Pochta");
define("NOWVER", "1.3");
define("DATENOW", date("Y-m-d H:i:s"));
define("BASIC_LOG", "pochta");
define("BASIC_PASS", "superpochta");
		...
		if (eregi ("^[0-9]{4,8}$", $account)) {
			if (eregi ("(^[0-9]{1,6})([.])([0-9]{1,2}$)", $sum)) {
				...
				if ($payment_ext_number != $txn_id){
					#зачисляем 
					$pay=$tinsp->APIAddCash($GUID, $sum, "Пополнение через свободную кассу", $txn_id, SENDER);
					if ($pay[0]=="0") {
...
EOF;
	...
	fileopen("".DATENOW."	".$_SERVER['REMOTE_ADDR']."	key:[{$txt}] val:[{$txt_val}] \n");	
