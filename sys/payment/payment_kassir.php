...
#OPTIONS
#SENDER определяется ниже
define("DATENOW", date("Y-m-d H:i:s"));
define("BASIC_LOG", "kassiranet");
define("BASIC_PASS", "superkassiranet");
...
if(!each(array_diff_key($Array1, $Array2))){
   ...
   /*БАНКОВСКИЕ КАРТЫ ПОД ДРУГИМ ОПЕРАТОРОМ*/
	if ($term=="855") {
		define("SENDER", "Pskb_bank");
	}else{
		define("SENDER", "Kassira.net");
	}
   	...
   	if (eregi ("(^[0-9]{3,8}$)", $cid)) {
		...
		OutPut(SENDER, $id=NULL, -1, "Personal account $cid incorrect format", $cid, $uact, $term, $trans, $sum, "", DATENOW);			
	}
}else{
