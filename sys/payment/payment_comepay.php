<?php
/* comepay to simple-billing
(с)Bobrov Nikita
icq 332277202 */

#OPTIONS
define("SENDER", "ComePay");
define("NOWVER", "1.3");
define("DATENOW", date("Y-m-d H:i:s"));
define("BASIC_LOG", "comepay");
define("BASIC_PASS", "supercomepay");
	...
	if (eregi ("(^[0-9]{1,20}$)", $txn_id)) {
		...
		if (eregi ("^[a-zA-Z0-9.-]{4,32}$", $account)) {
			if (eregi ("(^[0-9]{1,6})([.])([0-9]{1,6}$)", $sum)) {
				...
				if ($basic_account == ""){
					OutPut($command, NOWVER, SENDER, $id=NULL, 5, "Номер договора $account не найден", "", $txn_id, $account, $sum, DATENOW, DATENOW);
				}elseif ($basic_account != ""){
					OutPut($command, NOWVER, SENDER, $id=NULL, 0, "Номер договора $account найден", "", $txn_id, $account, $sum, DATENOW, DATENOW);
				}
			}else{
				OutPut($command, NOWVER, SENDER, $id=NULL, 300, "Некорректный формат суммы", "", $txn_id, $account, $sum, DATENOW, DATENOW);
   			}
		}else{
		...
		if (eregi ("^[a-zA-Z0-9.-]{4,32}$", $account)) {
			if (eregi ("(^[0-9]{1,6})([.])([0-9]{1,2}$)", $sum)) {
				...
				if ($payment_ext_number != $txn_id){
					#зачисляем 
					$pay=$tinsp->APIAddCash($GUID, $sum, "Оплата через терминал comepay", $txn_id, SENDER);
					if ($pay[0]=="0") {
...
EOF;
	...
	#log biling
	if ($res!="0") {
		$tinsp->APICreatEvent("pay_error", $GUID, "", "", $res, "Sender: ".$sender.", error:".$res.", order = ".$account.", sum = ".$sum."");
	}
}
?>