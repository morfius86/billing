			...
			if (isset($HotSpotTarifProfile[$Nas2SectorValueValue[0]])){	
				foreach($HotSpotTarifProfile[$Nas2SectorValueValue[0]] as $TarHotSpotKey => $TarHotSpotVal){		
					...
					if (isset($HotSpotProfileArray[$TarHotSpotKey])){
						...
						if((string)$HotSpotProfileArray[$TarHotSpotKey]["rate-limit"]!="0m/0m"){
							$edit_profile_attr["rate-limit"] = "0m/0m";				
						}
						
						# если для тарифа есть queue type, то отредактируем
						if (isset($HotSpotQueueTypes[$TarHotSpotKey])) {
							if((string)$HotSpotProfileArray[$TarHotSpotKey]["insert-queue-before"]!="bottom"){
								$edit_profile_attr["insert-queue-before"] = "bottom";				
							}
							
							if((string)$HotSpotProfileArray[$TarHotSpotKey]["queue-type"]!=$HotSpotQueueTypes[$TarHotSpotKey]["name"]){
								$edit_profile_attr["queue-type"] = $HotSpotQueueTypes[$TarHotSpotKey]["name"];				
							}
						}
						
						if (count($edit_profile_attr)>0){
							$edit_profile_attr["numbers"] = $HotSpotProfileArray[$TarHotSpotKey]["id"];
							$tinsp->nas->comm("/ip/hotspot/user/profile/set", $edit_profile_attr);
							# log
